...
export class TransactionController {
     ...
     *     summary: List Transaction Logs
     *     description: Retrieve a list of transaction API logs with optional filtering.
     *     produces:
     *       - application/json
     *     security:
     *       - bearerAuth: []
    ...
    async listTransactionLogs(req, res, next) {
        ...
        try {
            ...
            const result = await transactionServices.paginateTransactionLogList(validatedBody);
    ...
    async viewTransactionLog(req, res, next) {
        const validationSchema = Joi.object({
        ...
        try {
            const validatedBody = await validationSchema.validateAsync(req.query);
            let result = await transactionServices.findTransactionLog({ id: validatedBody.logId });
    ...
    async getTransactionLogsStats(req, res, next) {
        const validationSchema = Joi.object({
            startDate: Joi.string().optional().allow(''),
            endDate: Joi.string().optional().allow(''),
        });
        ...
        try {
            const validatedBody = await validationSchema.validateAsync(req.query);
            const result = await transactionServices.paginateTransactionLogList({
                ...validatedBody,
                limit: '1'
            });
